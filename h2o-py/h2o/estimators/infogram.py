...
class H2OInfogram(H2OEstimator):
    ...
    def __init__(self,
                 ...
                 cmi_threshold=0.1,  # type: float
                 relevance_threshold=0.1,  # type: float
        ...
        :param cmi_threshold: conditional information threshold between 0 and 1 that is used to decide whether a
               predictor's conditional information is high enough to be chosen into the admissible feature set.  Default
               to 0.1
               Defaults to ``0.1``.
        :type cmi_threshold: float
        :param relevance_threshold: relevance threshold between 0 and 1 that is used to decide whether a predictor's
               relevance level is high enough to be chosen into the admissible feature set.  Default to 0.1
               Defaults to ``0.1``.
    ...
    @property
    def cmi_threshold(self):
        """
        conditional information threshold between 0 and 1 that is used to decide whether a predictor's conditional
        information is high enough to be chosen into the admissible feature set.  Default to 0.1

        Type: ``float``, defaults to ``0.1``.
        """
        return self._parms.get("cmi_threshold")

    @cmi_threshold.setter
    def cmi_threshold(self, cmi_threshold):
        assert_is_type(cmi_threshold, None, numeric)
        self._parms["cmi_threshold"] = cmi_threshold

    @property
    def relevance_threshold(self):
        """
        relevance threshold between 0 and 1 that is used to decide whether a predictor's relevance level is high enough
        to be chosen into the admissible feature set.  Default to 0.1

        Type: ``float``, defaults to ``0.1``.
        """
        return self._parms.get("relevance_threshold")

    @relevance_threshold.setter
    def relevance_threshold(self, relevance_threshold):
        assert_is_type(relevance_threshold, None, numeric)
        self._parms["relevance_threshold"] = relevance_threshold
